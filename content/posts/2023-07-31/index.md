...
Next, we created two IAM roles in each managed account. One role has the `PutEvent` permission, used by the Event Rule to send events to the event bus in the Security account. The second role has the `ec2:CreateTags` and `ec2:DescribeVolumes` permission, and its trust policy allows our Lambda's execution role to assume it. CloudFormation is again used to help deploy these to all accounts in scope. It's best to deploy this CloudFormation first, as you can then reference the `PutEvent` role in your CloudFormation for deploying the Event Rule itself. 
